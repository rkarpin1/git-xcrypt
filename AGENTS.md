...
`git-xcrypt` is a Rust CLI that transparently encrypts selected files on commit and decrypts them on checkout. The crate is split into `src/lib.rs` (logic) and a thin `src/main.rs` (arguments, exit codes). S-01 through S-04 have shipped: `init` sets the repository up, `process` serves git's long-running filter protocol, `sync` regenerates the cosmetic `.gitattributes` lines, `export-key` / `import-key` / `unlock` carry a key to another machine and open a clone with it, and `lock` closes a repository and deletes its key. `status` and `diff` are still unbuilt, so do not register or reference a subcommand before it exists. The decisions live under `context/foundation/`.
