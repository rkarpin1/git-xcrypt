...
2. **Co chroni użytkownika przed utratą jedynego pliku klucza?** — kontrargument przyjęty przy FR-008; brak kopii zapasowej klucza oznacza utratę całej historii sekretów. **Nadal otwarte co do zasady** (kopii zapasowej nie ma), ale implementacja dołożyła cztery zabezpieczenia, żeby nie czytać tego jako „nic nie zrobiono": `export-key` mówi wprost, że ten plik jest jedyną drogą powrotu do historii; `lock` wymaga wpisania `yes`, podaje `key_id` (nigdy klucz) i kieruje do `export-key`; `lock` odmawia przy niezacommitowanych zmianach, czego `--yes` nie obchodzi; `lock` odmawia także, gdy istnieją inne podłączone worktree czytające ten sam klucz. Owner: użytkownik.
