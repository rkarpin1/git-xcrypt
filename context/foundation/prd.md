...
- FR-011: Użytkownik może zainstalować gotową binarkę dla swojej platformy bez kompilowania i bez dodatkowych bibliotek. Priorytet: must-have
  ...
  >
  > **Stan na 2026-08-05 — odpowiedziana połowa.** Podpis: rozstrzygnięty na atestacje proweniencji GitHuba (`zalozenia.md` §Otwarte decyzje poz. 14), więc każde archiwum mówi, z którego commita i przebiegu powstało. Odtwarzalność: **odrzucona 2026-08-11, decyzją właściciela** — do tej daty „świadomie otwarta". Build nie jest odtwarzalny i nie będzie, więc nikt z zewnątrz nie potwierdzi, że te bajty wynikają z tego kodu; `README.md` mówi to wprost i kieruje po tę gwarancję do budowania ze źródeł. Kontrargument FR-011 zostaje zatem **przyjęty w połowie i w połowie zaakceptowany jako koszt**, a nie odpowiedziany. Pomiary, które to uzasadniają — `trim-paths` nadal wymaga nightly, katalog domowy budującego i tożsamość kompilatora wchodzą do binarki, ale sam build jest lokalnie deterministyczny — oraz warunki powrotu do tematu: `zalozenia.md` §Otwarte decyzje poz. 14.

## Non-Functional Requirements

- Codzienne operacje gita (dodanie do indeksu, commit, checkout, przełączenie gałęzi) na typowym pliku konfiguracyjnym nie są odczuwalnie wolniejsze niż w repozytorium bez szyfrowania. **Próg liczbowy rozstrzygnięty 2026-08-06** (Open Question 5) — trzy budżety, bo mierzą trzy różne rzeczy; **czwarty doszedł 2026-08-07**, bo pilnuje czwartej:
  - **plik przepuszczany: ≤ 25 µs** ponad repozytorium bez filtra. To jest budżet chroniący całe repozytorium, a nie sekrety w nim: linia catch-all przepuszcza przez filtr **każdy** plik, więc regresja tutaj obciąża każdy projekt przy każdym `git add`, niezależnie od tego, czy cokolwiek szyfruje.
  - **plik szyfrowany: ≤ 30 µs** ponad to samo, przy pliku rzędu 4 KB.
  - **szyfr: ≤ 2 ns/bajt** (~500 MB/s). **Zastrzeżenie dopisane 2026-08-11: ten jeden próg jest skalibrowany na `aarch64-apple-darwin` i nie obowiązuje wszędzie.** Zmierzone na deweloperskiej maszynie Windows x86-64: te same 8 MB przez `diff` idzie **3,18 ns/B**, przy czym `aes::hardware_accelerated()` odpowiada tam `true` — czyli backend jest sprzętowy, a różnicę robią uruchomienie procesu i 8 MB przez potok, które na tej platformie są sporą częścią pomiaru. Pozostałe trzy budżety przechodzą tam z zapasem. Wniosek do stosowania: ten próg czytać jako liczbę bezwzględną **tylko** na platformie, na której go zmierzono, a poza nią porównywać przebieg z poprzednim przebiegiem na **tej samej** maszynie.
  - **stos atrybutów: ≤ 10 ms** na zbudowanie i pierwszą odpowiedź, na drzewie testowym z 3000 katalogów × 10 plików leżących **poza** łańcuchem przodków pytanej ścieżki — dodany 2026-08-07, gdy odkrywanie źródeł w `AttributeResolver` przeszło z pełnego spaceru po drzewie na sondowanie przodków. Pilnuje jedynej zmierzonej pozycji, która skalowała się z liczbą plików *nieśledzonych*: pełny spacer kosztował na tym samym drzewie **43,6 ms** (a 220 ms na drzewie z 480 000 plików), leniwe sondowanie **0,02 ms**. Zapas jest tu szerszy niż 2–4× pozostałych budżetów, świadomie: to niemal czyste I/O systemu plików, gdzie zimny cache skacze mocniej niż scheduler, a awaria, którą budżet łapie, leży 4× nad progiem i rośnie liniowo z wpisami drzewa, podczas gdy koszt leniwy nie rośnie wcale.
  - Miara: **minimum** z 5 przebiegów, nie mediana — każde źródło szumu może przebieg wyłącznie spowolnić, więc najszybszy jest najmniej skażony. Egzekwuje `tests/performance.rs`, świadomie `#[ignore]`, uruchamiane `cargo test --release --test performance -- --ignored --nocapture`.
  - **Poza progiem:** `status` i `diff`. Nie leżą na ścieżce gorącej, użytkownik wywołuje je świadomie, a koszt `status` zależy od liczby obiektów w historii — wrzucenie ich tutaj rozmyłoby budżet, który ma pilnować operacji wykonywanych przy każdym zapisie.
  - Zmierzone 2026-08-06 przy tych progach: przepuszczanie **0–6 µs**, szyfrowanie **4–9 µs/plik**, szyfr **1,18 ns/B** (849 MB/s). Zapas ~2–4×, dobrany tak, żeby przebieg przeżył zwykły szum, a mimo to upadł na prawdziwej regresji — sprawdzone trzema mutacjami, każda czerwona.
  - **Sprostowanie z 2026-08-06 do liczb, które stały tu wcześniej.** Poprzedni zapis podawał jako podstawę progu: „bez filtra **540 ms**, filtr długożyjący **596 ms** (+10%)". Te 596 ms to **przepuszczanie**, nie szyfrowanie — zmierzone ponownie: 562 ms bez filtra, 592 ms z filtrem i pustą deklaracją, 680 ms przy szyfrowaniu wszystkiego. Kto wyprowadziłby stąd budżet szyfrowania, ustawiłby go na liczbie mierzącej co innego. Liczba, która się broni bez zmian, to dyskwalifikacja procesu na plik: **12 105 ms**, czyli 22×.
...
- Repozytorium zaszyfrowane na jednej z trzech wspieranych platform odszyfrowuje się bez różnic na dwóch pozostałych, włącznie z zachowaniem końców linii.
  - **Doprecyzowane 2026-08-11, po zmierzeniu, że przez rok tego nie spełniało.** Ścieżka zadeklarowana dostawała przy `core.autocrlf` false/brak i `core.eol` brak — czyli w domyślnej konfiguracji gita — końce linii **platformy**, więc ten sam plik miał CRLF na Windows i LF na Linuksie, podczas gdy jego niezadeklarowany bliźniak był na obu identyczny. Od tej daty w tej konfiguracji wypisywane są bajty tak, jak je zapisano, i wymóg trzyma. Zostaje jedna świadoma granica, za to jednakowa na każdej platformie: plik przyniesiony z CRLF wraca jako LF, bo normalizuje go `clean`, a nagłówek nie zapisuje, jaki koniec linii tam był. Pomiary, tabela i powód: `zalozenia.md` §Końce linii.
...
5. ~~**Jaki jest liczbowy próg dla NFR wydajnościowego?**~~ **Rozstrzygnięte 2026-08-06: trzy budżety — 25 µs na plik przepuszczany, 30 µs na plik szyfrowany, 2 ns/bajt na szyfr**, mierzone jako minimum z 5 przebiegów. Pełny zapis w §Non-Functional Requirements; egzekwuje `tests/performance.rs`, świadomie `#[ignore]`. **Dopisek z 2026-08-07:** doszedł czwarty budżet — 10 ms na zbudowanie stosu atrybutów na drzewie, którego masa leży poza łańcuchem przodków — razem ze zmianą `attribute-stack-walks-ancestors`; zapis obok trzech pozostałych w §NFR. Owner: użytkownik. Blokuje: nic.
   - **Dlaczego `#[ignore]`, a nie bramka CI.** Dwa powody, oba zmierzone. Liczby mają sens wyłącznie w buildzie `--release`, a `cargo test` domyślnie buduje debug. I ważniejsze: rozrzut na cichej maszynie deweloperskiej sięgnął **11%** bazy, a współdzielony runner jest gorszy — migotliwa bramka to bramka, której użytkownik uczy się nie czytać, czyli ten sam tryb awarii, przed którym projekt broni się przy ostrzeżeniach filtra. Próg egzekwuje więc człowiek, świadomie, przy zmianach w `commands/filter.rs`, `rules/decide.rs` i `crypto/`; wymóg zapisany w `AGENTS.md`.
   - **Trzy budżety, nie jeden, i to jest wynik pomiaru, nie estetyka.** Pierwsza wersja miała jeden próg wyprowadzany z dwóch rozmiarów plików przez policzenie nachylenia. Odczyt per-bajt wychodził wtedy jako **różnica różnic** czterech zaszumionych czasów i skakał między 0,24 a 0,67 ns/B na **niezmienionym** kodzie. Bramka, której wskazanie rusza się 3× samo z siebie, nie może zaraportować niczego mniejszego.
   - **`git add` w ogóle nie widzi kosztu na bajt — zmierzone.** Przy 32 plikach po 4 MB baza bez filtra dała 2386 ms, a ramię szyfrujące 2358 ms, czyli *szybciej*: nasze 0,85 ns/B to poniżej 5% liczby, od której się odejmuje, bo git na nieściśliwej treści robi ~18 ns/B (odczyt, zlib, zapis). Ta wersja testu przechodziła **pusto**, raportując `0.00 ns/B`. Budżet na bajt mierzy się więc przez `diff`, gdzie gita nie ma w pętli — tym samym przyrządem, którym mierzono backend aarch64 (do 2026-08-11 włączany flagą `--cfg aes_armv8`, dziś wybierany przez `aes` 0.9 samodzielnie).
6. **Czy któreś FR ma być `nice-to-have`?** — **odłożone świadomie 2026-08-06, decyzja właściciela: pomijamy.** Nie zmienia niczego praktycznego: wszystkie 11 wymagań jest dostarczonych, więc przepriorytetyzowanie nie zdjęłoby ani nie dodało pracy. Zostaje wyłącznie zastrzeżenie do czytania oceny gotowości — `must-have` przy każdej pozycji zostało **odziedziczone po braku odpowiedzi** podczas kształtowania, a nie nadane. Owner: użytkownik. Blokuje: nic.
...
8. **Historyjki użytkownika dla pozostałych FR** — **odłożone świadomie 2026-08-06, decyzja właściciela: pomijamy.** To luka w *zapisie*, nie w pokryciu: kryteria akceptacji FR-002, FR-007, FR-009 i FR-010 istnieją jako scenariusze realnego użycia (mapa w komentarzu nad §User Stories), i są od historyjek bogatsze — obejmują przypadki, których Given/When/Then by nie ujął: podłączony worktree, klon płytki, repozytorium SHA-256. Napisanie ich nie dodałoby ani jednej asercji. Owner: użytkownik. Blokuje: nic.
