...
- **Zakres pozostały:** sama publikacja. Podpisywanie artefaktów (kontrargument z PRD FR-011 wymaga prawdziwej tożsamości podpisującej, nie kroku w workflow), tap Homebrew, publikacja na crates.io, pierwszy tag `v0.1.0`. Żaden z workflow nie był jeszcze uruchomiony przez GitHub Actions — do tej pory bramka jakości chodziła wyłącznie lokalnie, na macOS.
- **Unknowns:**
...
6. **Co chroni użytkownika przed utratą jedynego pliku klucza?** — **wciąż otwarte**, ale nie puste. Zaimplementowane do 2026-08-04: `export-key` odmawia cichego nadpisania istniejącego pliku klucza (`--force` jest jawną zgodą) i odmawia zapisu wewnątrz katalogu gita; `lock` wymaga wpisania `yes`, wypisuje `key_id` (nigdy klucza) i odsyła do `export-key`; `lock` odmawia przy niezacommitowanych zmianach oraz gdy istnieje inny podłączony worktree tego samego repozytorium. Czego nadal nie ma: żadnego mechanizmu wymuszającego istnienie kopii zapasowej poza maszyną. — Właściciel: użytkownik. Blokuje: nic.
...
- **S-01: commitować plik i dostać ciphertext w repo, plaintext w katalogu roboczym** — Ukończono 2026-08-04 → `context/changes/transparent-encrypt-decrypt/` (status `impl_reviewed`, dwa przebiegi przeglądu). Lekcja: ładunek protokołu pkt-line to bajty, nie tekst — każde przejście ścieżki przez `String` albo przez `trim_end()` przesuwało plik na inny wzorzec i przepuszczało plaintext przy `git add` z kodem `0`. Drugi przebieg znalazł błąd wprowadzony przez naprawę z pierwszego, więc jeden przebieg by nie wystarczył.
- **S-02: wskazać pliki do szyfrowania w składni `.gitignore`** — Ukończono 2026-08-04 → `context/changes/gitignore-style-config/` (status `impl_reviewed`, dwa przebiegi przeglądu). Lekcja: „linia kosmetyczna" to ocena, nie fakt — wzorzec katalogowy generowany do `.gitattributes` sięgał płycej niż ten sam wzorzec w filtrze, więc `-text` znikało dokładnie tam, gdzie chroni ciphertext przed `core.autocrlf`. Każdy werdykt trzeba było sprawdzić przeciw `git check-attr`, nie przeciw własnemu rozumieniu składni.
- **S-03: odzyskać sekrety po klonie na drugiej maszynie** — Ukończono 2026-08-04 → `context/changes/key-export-and-unlock/` (status `impl_reviewed`, dwa przebiegi przeglądu). Lekcja: dwa moduły zapisujące pliki atomowo rozjechały się w tym, co robią bezpiecznie — ten, którym szedł klucz główny, otwierał przewidywalną nazwę bez `O_EXCL`, więc dawał się podstawić dowiązaniem. Jeden wzorzec zapisu na cały projekt, nie dwa podobne.
