...
| E      | Klucze i odbiorcy        | `S-03` → `S-10` → `S-09`; `S-10` → `S-11` | **Poza v0.1.** `S-10` wprowadza keyring oraz rotację do przodu; `S-09` dodaje tożsamości użytkowników i koperty dla wszystkich kluczy keyringu; `S-11` jest oddzielną operacją awaryjną przepisującą historię. Format danych jest gotowy i nie wchodzi do zakresu. |
...
**Poza v0.1 stoją dziś trzy elementy:** `S-10` i zależny od niego `S-11`, zapisane 2026-08-15 na prośbę właściciela, oraz `S-09`. Najpierw `S-10` wprowadza keyring i rotację do przodu; następnie `S-09` może tworzyć koperty dla jego kluczy. `S-11` pozostaje osobną, awaryjną operacją przepisywania historii. Żaden z tych elementów nie jest zaplanowany na konkretne wydanie. Dla `S-09` format koperty to `crypto_box`, ale nadal blokuje go cięższa niewiadoma: **gdzie w ogóle mieszka klucz prywatny użytkownika**, skoro pojęcie tożsamości w tym produkcie nie istnieje.

| Roadmap ID | Change ID                    | Sugerowany tytuł zadania                            | Gotowe do `/10x-plan` | Uwagi                                       |
| ---------- | ---------------------------- | --------------------------------------------------- | --------------------- | ------------------------------------------- |
| S-10       | forward-key-rotation         | Rotacja klucza do przodu                            | **nie**               | Poza v0.1, zapisane 2026-08-15. Wielokluczowy keyring bez przepisywania historii; odcina stary klucz tylko od nowych sekretów |
| S-11       | rewrite-history-key-rotation | Rotacja klucza z przepisaniem historii              | **nie**               | Poza v0.1, zapisane 2026-08-15. Zależy od S-10; awaryjny force-push nie usuwa starych klonów ani nie zastępuje rotacji sekretów |
| S-09       | per-user-keys                | Obsługa kluczy użytkowników                         | **nie**               | Poza v0.1, zapisane 2026-08-11. Zależy od S-10; format koperty (`crypto_box`) rozstrzygnięty, blokuje jeszcze brak pojęcia tożsamości użytkownika |
