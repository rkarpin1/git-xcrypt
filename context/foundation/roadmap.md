...
- **Unknowns:**
  ...
  - ~~Czy `stderr` filtra jest wystarczająco widoczny w oknie narzędziowym Git w JetBrains, żeby ostrzeżenie nie ginęło?~~ **Rozstrzygnięte 2026-08-12: tak, widać je.** Sprawdzone przez właściciela w RustRoverze, na repozytorium ustawionym w ten stan celowo: plik leżący w `HEAD` jawnie (19 bajtów) i objęty `.git-xcrypt` dopiero od następnego commita, więc filtr szyfruje go przy każdym porównaniu i ostrzeżenie „first time it is being encrypted, and HEAD already holds it in the clear" leci już przy `git status` i `git diff`, a nie dopiero przy commicie — co czyni je łatwiejszym do zauważenia, niż zakładano przy stawianiu pytania. Pytanie wisiało od 2026-08-04, bo wymagało człowieka przy IDE i nie dawało się sprawdzić automatycznie ani zgadnąć. Konsekwencja dla pozostałych trzech automatycznych ostrzeżeń filtra (nieaktualna sekcja zarządzana, stratna normalizacja, `eol=` nie do przyjęcia przez treść) jest ta sama, bo wszystkie idą tą samą drogą. — Właściciel: użytkownik. Blokuje: nic.
...
- **Co z tego elementu było otwarte i zostało domknięte 2026-08-11:** publikacja na crates.io — **wykonana** przy wydaniu `0.1.1`, 59 plików, 337,3 KiB skompresowane. Sprawdzona jak obcy, nie założona: `cargo install git-xcrypt --locked` z rejestru buduje się i instaluje, a zainstalowana binarka podaje `git-xcrypt 0.1.1` i przechodzi `init` na świeżym repozytorium. Pierwsza próba odbiła się od crates.io kodem 400 („a verified email address is required"), więc token działał, a brakowało weryfikacji konta; nic wtedy nie wylądowało w rejestrze — potwierdzone zapytaniem, nie założeniem. **To jedyny kanał pakietowy, jaki powstanie:** tap Homebrew, bucket Scoop, WinGet i Chocolatey zostały odrzucone tego samego dnia (`zalozenia.md` §Założenia techniczne). Publikacja nie była warunkiem wydania binarek, czyli tego, co obiecuje FR-011 — `cargo install` kompiluje ze źródeł i wymaga toolchaina, więc tamtą obietnicę spełniają nadal archiwa z Releases. Odtwarzalność builda **przestała być pozycją otwartą 2026-08-11** — odrzucona decyzją właściciela, z pomiarami i warunkami powrotu w `zalozenia.md` §Otwarte decyzje poz. 14. `README.md` nadal mówi wprost, że build nie jest odtwarzalny; zmienił się status pozycji, nie ostrzeżenie.
- **Status:** done
...
- **Zakres — co konkretnie trzeba zmienić.** Rozstrzygnięć nie zawiera i żadnej niewiadomej niżej nie zamyka; jest po to, żeby ktoś sięgający po ten element nie odkrywał dwóch rzeczy w połowie roboty: że **tożsamość użytkownika jest elementem osobnym i wcześniejszym**, a nie detalem kopert, i że **`lock` przestaje być operacją nieodwracalną**.
  - **Do zbudowania od zera:**
    1. **Tożsamość użytkownika** — para kluczy na osobę, leżąca **poza** repozytorium. Dochodzi nowy plik i nowy **zamrożony** format, z tą samą dyscypliną co `crypto/keyfile.rs` (magic, bajt wersji, wektory), bo będzie leżał u ludzi i w kopiach zapasowych; decyzja, gdzie mieszka i czym jest chroniony — a tu wraca zapisana granica, że **na Windows nie ustawiamy ACL**, więc klucz prywatny użytkownika dziedziczy dokładnie ten sam problem co klucz repozytorium; oraz komenda, która go tworzy. Problem transportu **nie znika, tylko przesuwa się o poziom wyżej**: zamiast pliku klucza repozytorium trzeba przenieść tożsamość.
    2. **Format koperty** — `sealed(klucz główny)` plus identyfikator odbiorcy, zamrożony tak samo. To tutaj ląduje otwarta decyzja 1.
    3. **Układ katalogu `.git-xcrypt-keys/`** — jeden plik na odbiorcę. Wprowadza w tym produkcie nową klasę zachowania: komendę, która **brudzi drzewo robocze plikiem wymagającym commita**. Dziś robią to wyłącznie `init` i `sync`.
  - **Do zmiany w istniejącym:**
    4. **`init`** — po wygenerowaniu klucza głównego musi zapieczętować go dla tożsamości inicjującego i zapisać pierwszą kopertę, inaczej model się nie bootstrapuje. Czyli `init` po raz pierwszy **wymaga** istniejącej tożsamości.
    5. **`unlock`** — czwarte i domyślne wejście obok dzisiejszych trzech (`unlock <plik>`, `--key`, `--key-only`): bez argumentu znajdź kopertę, którą umiem otworzyć. Tamte zostają jako droga ratunkowa.
  ...
  - **Konsekwencja dla `lock`, zapisana osobno, bo z nazwy funkcji nie wynika.** Cała jego konstrukcja bezpieczeństwa stoi na zdaniu **„to jedyna kopia klucza"**: ostrzeżenie z `unlock WILL NOT UNDO THIS`, potwierdzenie przez wpisanie `yes`, `key_id` zamiast klucza, PRD Open Question 2 i sekcja `README.md` „The key file is the only copy — back it up yourself". Przy zacommitowanych kopertach to zdanie jest **fałszywe** — każdy uprawniony otworzy repozytorium z powrotem. `lock` przestaje więc być nieodwracalny, ostrzeżenie trzeba przepisać, a pytanie „co chroni przed utratą klucza" przenosi się z klucza repozytorium na **klucz tożsamości**, którego kopii nadal nikt nie robi za użytkownika. Zmiana na lepsze, ale dotyka najostrożniej pisanej części projektu — dwóch przebiegów przeglądu, testów scenariuszowych i trzech dokumentów.
  - **Kolejność:** tożsamość **przed** kopertami, bo bez odpowiedzi, gdzie mieszka klucz prywatny, format koperty nie ma dla kogo powstać — kto zacznie od wyboru crate'a, zacznie od środka. Potem wybór formatu, `init`, `unlock`, `add-user`; `lock` i dokumentacja na końcu, jednym ruchem.
- **Unknowns:**
  ...
  - **Gdzie leży klucz prywatny użytkownika i jak trafia na drugą maszynę?** Dziś pojęcie tożsamości użytkownika **nie istnieje** — tożsamością jest sam klucz repozytorium. Koperty wymagają pary kluczy na osobę, więc problem transportu, który produkt świadomie zostawia użytkownikowi, przesuwa się o jeden poziom, a nie znika. **Świadomie nierozstrzygnięte — właściciel odłożył to 2026-08-11, a postawione ponownie 2026-08-12 odłożył drugi raz; decyzja ma zapaść w ramach tego elementu, a nie przed nim.** Zapisane, żeby odróżnić „nikt się nad tym nie zastanawiał" od „przemyślane i odłożone": pytanie było postawione dwa razy i wróci wtedy, gdy `S-09` zostanie wzięty do realizacji. Do rozstrzygnięcia wtedy: gdzie plik mieszka, czym jest chroniony (na Windows nie ustawiamy ACL, więc dziedziczy problem klucza repozytorium), czy jest jeden na osobę czy jeden na maszynę, i jak trafia na drugą.
    - **Warianty rozważone 2026-08-12, żeby powrót nie zaczynał się od zera.** *Jedna tożsamość na osobę* — plik w katalogu konfiguracyjnym użytkownika; transport **nie znika**, tylko przenosi się o poziom wyżej, za to płaci się go raz na życie, a nie raz na repozytorium. *Jedna tożsamość na maszynę* — każda generuje swoją przy pierwszym użyciu i jest osobnym odbiorcą, więc transportu nie ma w ogóle; koszt jest w bootstrapie, bo nowa maszyna nie otworzy repozytorium, dopóki nie doda jej maszyna **już** odblokowana, czyli przy jednej maszynie wraca plik klucza. *Oparcie się o istniejący klucz SSH* (`ed25519` przeliczony na X25519) — zero nowych plików i zero transportu, ale przeliczenie jest **konstrukcją składaną przez nas**, wprost przeciw twardej regule „nigdy nie składamy własnych konstrukcji z prymitywów", więc wejdzie w grę tylko wtedy, gdy właściciel świadomie tę regułę otworzy — osobna decyzja, tak samo jak przy licencji sequoi. — Właściciel: użytkownik. Blokuje: tak, start elementu.
...
Backlog v0.1 jest **pusty** — `S-07` zamknięty 2026-08-07 wydaniem `v0.1.0`, jako ostatni. `S-08` wszedł przed nim 2026-08-04, w wymaganej kolejności: reguła tekst/binarny zamraża się z pierwszym publicznym wydaniem. Co zostało poza roadmapą v0.1: publikacja na crates.io — **wykonana 2026-08-11 przy wydaniu `0.1.1`** i jedyny kanał pakietowy, jaki powstanie, bo tap Homebrew i pozostałe menedżery odrzucono tego samego dnia — plus pozycje z `## Parked`.

**Poza v0.1 stoi dziś jeden element: `S-09`**, zapisany 2026-08-11. Nie jest gotowy do `/10x-plan` i nie jest zaplanowany na żadne wydanie. Z dwóch niewiadomych blokujących start jedna odpadła 2026-08-11 — format koperty to `crypto_box` — i została ta druga, cięższa: **gdzie w ogóle mieszka klucz prywatny użytkownika**, skoro pojęcie tożsamości w tym produkcie nie istnieje.

| Roadmap ID | Change ID                    | Sugerowany tytuł zadania                            | Gotowe do `/10x-plan` | Uwagi                                       |
| ---------- | ---------------------------- | --------------------------------------------------- | --------------------- | ------------------------------------------- |
| S-09       | per-user-keys                | Obsługa kluczy użytkowników                         | **nie**               | Poza v0.1, zapisane 2026-08-11. Format koperty rozstrzygnięty tego samego dnia (`crypto_box`); blokuje jeszcze brak pojęcia tożsamości użytkownika. Format danych gotowy, nie zmienia się |
...
| S-07       | cross-platform-binaries      | Binarki dla Windows, macOS i Linuksa                | zrobione              | Wydane 2026-08-07 tagiem `v0.1.0` (przebieg `31211254819`): pięć archiwów z sumami i atestacją, weryfikacja odbiorcy sprawdzona. Otwarte świadomie i poza wydaniem: crates.io; tap Homebrew odrzucony 2026-08-11 |
