...
## Rozstrzyganie atrybutów przez `status` — rozstrzygnięte 2026-08-04

`status` nie wypisuje już podejrzanych linii `.gitattributes` — **odtwarza stos atrybutów gita i pyta o faktyczną wartość `filter`** dla każdej zadeklarowanej ścieżki w indeksie, dokładnie tak, jak odpowiada `git check-attr filter`.
...
- **Werdykt:** ścieżka zadeklarowana, dla której git nie rozwiązuje `filter=git-xcrypt`, jest **luką w konfiguracji** i kończy kodem `5`. Wartości `unset`, `unspecified`, `set` i obcy sterownik są tu równoważne — żadna z nich nie uruchamia naszego filtra.
