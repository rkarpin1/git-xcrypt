...
13. **Semantyka wzorców wobec `core.ignorecase` i normalizacji Unicode w nazwach plików.** Nadal **nierozstrzygnięta**, ale od 2026-08-05 **zmierzona**, a `status` przestał na nią milczeć.
    - **Wielkość liter — zmierzone na git 2.55 i APFS.** Git zwija wielkość liter przy `core.ignorecase = true` (domyślnie na macOS i w Git for Windows) **zarówno w `.gitignore`, jak i w `.gitattributes`**; przy `core.ignorecase = false` nie zwija w żadnym. Sprawdzone w obie strony: `.gitignore` z linią `secrety/` ukrywa katalog `Secrety/` tylko przy `true`, a `git check-attr text -- top.ENV` przy linii `*.env` odpowiada `unset` przy `true` i `unspecified` przy `false`. `src/config.rs` dopasowuje `Case::Sensitive` zawsze. Skutek zmierzony na prawdziwym repozytorium: `.git-xcrypt` z `*.env`, plik `top.ENV`, `git add -A` → **plaintext w bazie obiektów, kod 0**, a jednocześnie `git check-attr filter text diff` odpowiada `git-xcrypt`, `unset`, `git-xcrypt` — czyli **wygenerowana sekcja sięga dalej niż filtr**, co „Konstrukcja catch-all" zakazuje w obie strony. Na systemie plików nieczułym na wielkość liter użytkownik **nie ma jak zapisać różnicy**: obie pisownie to jeden plik.
    - **Dlaczego to nadal nie jest naprawione.** Naprawa po stronie selekcji znaczy czytanie `core.ignorecase` na ścieżce clean, a to ustawienie **nie jest wersjonowane** — to samo repozytorium szyfrowałoby wtedy inny zbiór plików na macOS niż na Linuksie. Dokładnie ten argument wyklucza `core.autocrlf` z ścieżki clean („Końce linii"). Naprawa po stronie renderingu jest niewykonalna: dopasowania atrybutów w gicie nie da się zawęzić per linia. Wybór między nimi to decyzja właściciela.
    - **Co się zmieniło.** `status` pyta `Config::selected_only_when_folding_case` o każdą śledzoną ścieżkę, której bajtowe dopasowanie nie wybiera, i przy `core.ignorecase = true` zgłasza takie ścieżki w sekcji `undetermined` — kod **`6`**, nie `5`: `5` mówi „rotuj sekret", a tutaj lekarstwem jest ustalenie pisowni i ponowne pytanie. Selekcja nie drgnęła ani o bajt. Testy: `config::tests::folding_case_is_asked_about_and_never_acted_on` oraz dwa w `tests/status_command.rs`, z których drugi pilnuje, żeby checkout czuły na wielkość liter nie zapalał bramki.
...
15. **Czy plik klucza dostaje na Windows własne ACL.** Dziś nie dostaje żadnego — patrz korekta w „Zarządzanie kluczami i użytkownikami". Zawężenie ACL do właściciela wymagałoby `windows-sys` i bloku `unsafe`, czyli złamania reguły „zero `unsafe`" wymuszanej przez `unsafe_code = "forbid"` w `Cargo.toml`, więc nie jest to poprawka jednej linii tylko decyzja o zakresie. Do rozstrzygnięcia przed pierwszym publicznym wydaniem, razem z poz. 14: obie dotyczą tego, co obiecujemy użytkownikowi Windows. Do tego czasu obowiązuje wpis w `README.md` §Known limitations, żeby brak ochrony był zapisaną granicą, a nie cichą.
