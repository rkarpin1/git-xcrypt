...
- **Sprzętowy AES na aarch64 włączany jawnie — rozstrzygnięte 2026-08-06.** `.cargo/config.toml` przekazuje `--cfg aes_armv8` dla `cfg(target_arch = "aarch64")`. Powód jest zmierzony: `aes` 0.8 wykrywa AES-NI na x86 i x86-64 sam, ale rozszerzenia kryptograficzne ARMv8 stoją za tą flagą, więc bez niej **każde** szyfrowanie i deszyfrowanie na Apple Silicon i na aarch64 Linuksie szło programowym backendem bitslice. Zmierzone na `aarch64-apple-darwin`, build `--release`, mediana z trzech przebiegów: blob 8 MB przez `git-xcrypt diff` **148 ms → 9 ms**, `git checkout` 200 plików po 87 KB **410 ms → 50 ms**, `git add -A` na tych samych plikach **480 ms → 110 ms**.
  - **Determinizmu to nie rusza i nie mogło ruszyć.** AES to AES — oba backendy liczą tę samą funkcję. Dowodem jest jednak pomiar, nie argument: wyjście `diff` na blobie 8 MB ma po obu stronach tę samą sumę SHA-256, a zamrożone wektory formatu (`tests/format_vectors.rs`) przechodzą. Cały zestaw 123 testów przechodzi bez zmiany ani jednej linii kodu — to zmiana wyłącznie konfiguracji builda.
  - **Nie grozi „illegal instruction".** Na macOS i Linuksie `cpufeatures` pyta o rozszerzenia w czasie wykonania (`sysctlbyname`, `getauxval`), a każda inna platforma odpowiada `false` — czyli CPU bez rozszerzeń dostaje ten sam backend programowy co dotąd. Flaga jest zawężona do aarch64, bo tylko tam cokolwiek robi.
  - **Zapisana granica:** cargo czyta ten plik z katalogu, w którym go uruchomiono, więc flaga obejmuje workflow wydania, `cargo build` i `cargo install --path .` z klona — ale **nie** `cargo install git-xcrypt` z crates.io, gdzie na aarch64 zostaje backend programowy. Druga: wpis `[target.'cfg(…)']` przesłania `[build] rustflags` z globalnej konfiguracji użytkownika (`RUSTFLAGS` ze środowiska nadal wygrywa z obydwoma).
  - Sprawdzone kompilacyjnie na `aarch64-unknown-linux-musl`, `x86_64-unknown-linux-gnu` i `x86_64-pc-windows-msvc` — kompilacja dowodzi budowania, nie działania; uruchomiony był wyłącznie build macOS/arm64.
  - **Ślepa uliczka, żeby nikt nie odkrywał jej drugi raz: `build.rs` tego nie zamknie.** Skrypt budowania emituje `cargo::rustc-cfg` wyłącznie dla crate'a, który go niesie, a `aes_armv8` musi paść przy kompilacji **zależności** `aes` — dlatego działa `RUSTFLAGS`, a nie skrypt. Feature'a też nie ma: `aes` 0.8.4 trzyma ten backend za `cfg`, nie za przełącznikiem w `Cargo.toml`.
  - **Do sprawdzenia przy bumpie `aes-siv` do 0.8 (2026-08-06 stabilny to nadal 0.7.0, w drodze `0.8.0-rc.3`):** czy nowszy `aes` wykrywa rozszerzenia ARMv8 sam, bez `cfg`. Jeśli tak, luka crates.io zamyka się sama, a `.cargo/config.toml` znika razem z bumpem. Do tego czasu granica jest zapisana w `README.md` dwa razy — przy §Building, gdzie stoi instalujący, wraz z poleceniem `RUSTFLAGS="--cfg aes_armv8" cargo install git-xcrypt`, oraz w §Known limitations, gdzie czyta się listę granic. Dwa razy celowo: pierwsze miejsce sąsiaduje ze zdaniem „binarka jest samowystarczalna" i bez tego zastrzeżenia czytałoby się jak obietnica obowiązująca każdą drogą instalacji.
